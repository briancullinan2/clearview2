...
using EPIC.Annotations;
using EPIC.Utilities;
using EPICAnalysis;
using EPICClearViewDL.Customization;
...
namespace EPIC.Controls
{
	...
	public class CalibrationImage : ProcessingImage, INotifyPropertyChanged
	{
		...
		public ImageCalibrationEntity Result
		{
			get
			{
				return (ImageCalibrationEntity)base.GetValue(CalibrationImage.ResultProperty);
			}
			private set
			{
		...
		public CalibrationImage(ImageCalibrationEntity img) : this()
		{
			CalibrationImage <>4__this = this;
			if (img.Image.Capture != null)
			{
				CaptureEntity capture = img.Image.Capture.Capture;
			...
			{
				Start<ImageCalibrationEntity>.Work(() => <>4__this.ProcessOne(img), ThreadPriority.Lowest).OnComplete(delegate(ImageCalibrationEntity calib)
				{
					if (this._finished != null)
					{
						this._finished(this, calib);
					}
				}).OnException(delegate(Exception ex)
				{
					CalibrationImage.Log.Error("There was an error while processing the calibration results.", ex);
				}).RunNow();
			}
			else
			{
				this.Colorized = Compression.GetImageSource(img.Colorized);
				base.Dispatcher.Invoke<States>(() => <>4__this.State = ((!img.Failed) ? States.Complete : States.Failed));
			}
			this.Result = img;
		...
		private ImageCalibrationEntity ProcessOne(ImageCalibrationEntity img)
		{
			DeviceCalibrationSettingEntity settings = ClearViewConfiguration.Current.Device.CalibrationSettings.First<DeviceCalibrationSettingEntity>();
			base.Dispatcher.Invoke(delegate()
			{
				base.State = States.Processing;
				base.Value = 0.0;
			});
			using (Bitmap bitmap = Compression.DecompressImage(img.Image.Image))
			{
				ImageEntity colorized;
				ImageCalibrationEntity result = Maths.CheckCalibration(bitmap, settings, out colorized);
				if (result != null)
				{
		...
		public sealed override void StoreAndProcess(CaptureResults results)
		{
			...
			else
			{
				...
				CaptureEntity capture = results.Capture;
				if (capture != null)
				{
					this.Brightness = new int?(capture.Brightness);
					this.Gain = new int?(capture.Gain);
				...
				Start.Work(delegate()
				{
					this.StartProcessing(results);
				}, ThreadPriority.Lowest).OnException(delegate(Exception ex)
				{
					CalibrationImage.Log.Error("There was an error while processing the calibration results.", ex);
				}).RunNow();
			}
			...
			try
			{
				...
				foreach (Bitmap bitmap in results.Images)
				{
					if (ParallelWork.IsAnyWorkRunning())
					{
						...
						{
							base.State = States.Paused;
						}), new object[0]);
						ParallelWork.WaitForAllWork(new TimeSpan(0, 0, 0, 1));
						base.Dispatcher.BeginInvoke(new Action(delegate()
						{
							base.State = States.Processing;
						}), new object[0]);
					...
					{
						result = Maths.DoesImagePassValidation(bitmap, settings, out colorized);
					}
					bool flag = false;
		...
		private static readonly ILog Log = LogManager.GetLogger(typeof(CalibrationImage));

		// Token: 0x0400006B RID: 107
		public static readonly DependencyProperty ResultProperty = DependencyProperty.Register("Result", typeof(ImageCalibrationEntity), typeof(CalibrationImage), new PropertyMetadata(null));
		...
		public delegate void ProcessingFinishedHandler(CalibrationImage sender, ImageCalibrationEntity calibration);
