	...
	public partial class ProgressDialog : Window
	{
		...
		public static ProgressDialog Show(string titleMessage, params string[] stati)
		{
			...
			Application.Current.Dispatcher.Invoke(delegate()
			{
				...
				progressDialog.Title = titleMessage;
				progressDialog.Owner = (Application.Current.Windows.OfType<Window>().FirstOrDefault((Window x) => x.IsActive) ?? Application.Current.MainWindow);
		...
		public void Update(params Tuple<string, double>[] stati)
		{
			Application.Current.Dispatcher.Invoke(delegate()
			{
				...
				if (progressBar2 != null)
				{
					...
					progressBar2.Foreground = (Brush)new BrushConverter().ConvertFromString("#FF01D328");
		...
		private static readonly Timer ResetTask = new Timer
		{
			AutoReset = true,
			Interval = 2000.0,
			Enabled = false
