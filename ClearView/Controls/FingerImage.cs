...
namespace EPIC.Controls
{
	...
	public class FingerImage : ProcessingImage
	{
		...
		public ImageCalibrationEntity Calibration { get; set; }
		...
		public ImageAlignmentEntity Result
		{
			get
			{
				return (ImageAlignmentEntity)base.GetValue(FingerImage.ResultProperty);
			}
			private set
			{
		...
		public sealed override void StoreAndProcess(CaptureResults results)
		{
			...
			else
			{
				...
				{
					FingerImage.Log.Error("There was an error while processing the finger results.", ex);
				}).RunNow();
			}
		}
		...
		{
			Start.Work(new Action(this.StartProcessSectors), ThreadPriority.Lowest).OnException(delegate(Exception ex)
			{
				FingerImage.Log.Error("There was an error while processing the finger results.", ex);
			}).RunNow();
		}
		...
		{
			string finger = string.Empty;
			ImageAlignmentEntity alignment = null;
			base.Dispatcher.Invoke(delegate()
			{
				this.State = States.Processing;
				this.Value = 0.0;
			...
			if (alignment != null)
			{
				IEnumerable<ImageSectorEntity> enumerable = null;
				if (alignment.Image.ImageSectors.Count == 0 && this.Calibration != null)
				{
					enumerable = Maths.GetImageSectors(Compression.DecompressImage(alignment.Image.Image), Compression.DecompressImage(this.Calibration.Image.Image), alignment.Angle, alignment.CenterX, alignment.CenterY, this.Calibration.NoiseLevel, finger);
				}
				if (enumerable != null)
				{
					foreach (ImageSectorEntity imageSectorEntity in enumerable)
					{
						imageSectorEntity.Image = alignment.Image;
						imageSectorEntity.Save();
					}
		...
		private static readonly ILog Log = LogManager.GetLogger(typeof(ProcessingImage));
		...
		public static readonly DependencyProperty ResultProperty = DependencyProperty.Register("Result", typeof(ImageAlignmentEntity), typeof(FingerImage), new PropertyMetadata(null));
		...
		public delegate void ProcessingFinishedHandler(FingerImage sender, ImageAlignmentEntity alignment);
	}
}
